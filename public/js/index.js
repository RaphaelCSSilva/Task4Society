...
const urlRoutesEspacos = "http://localhost:3000/api/v1/espacos"

const profileBtns = async () => {
  var id_utilizador = localStorage.getItem('id_utilizador')
  const response = await fetch(`http://localhost:3000/api/v1/utilizadores/` + id_utilizador)
...
function refreshEspacos() {
  async function fetchAsync() {
      const renderUsers = document.getElementById("dataTable");
      let txt = "";
      const response = await fetch(urlRoutesEspacos);
      const espacos = await response.json();
      //criação de uma tabela para demonstração dos resultados recebidos  
      txt += "<tr><th>Id</th><th>Nome</th><th>Morada</th><th>Localidade</th><th>Distrito</th><th>Cod. Postal</th><th>Lotação</th><th>Gestor</th></tr>";
      //percorrer a variável users e por cada user cria a linha da tabela com os dados presentes   
      for (const espaco of espacos) {
          txt += "<tr><td>" + espaco.espacoId + "</td><td>" + espaco.nome + "</td><td>" + espaco.morada + "</td><td>" + espaco.localidade +
              "</td><td>" + espaco.distrito + "</td><td>" + espaco.codigoPostal + "</td><td>" + espaco.lotacao + "</td><td>" + espaco.gestorEspacoId +
              "</td> <td><a  href='javascript:deleteSpace(" + espaco.espacoId + ")'> <img src=./imagens/delete.jpg  width=35 height=35></img></a>" +
              "</td><td><a data-toggle=modal href=javascript:void(" + espaco.espacoId + ") onclick=openSpaceDetailsModal(" + espaco.espacoId + ");> <img  src=./imagens/edit.jpg  width=35 height=35></img></a></td> </tr>";
      }
...
function saveSpace() {
  ...
  fetch('http://localhost:3000/api/v1/espacos', {
...
function deleteSpace(espacoId) {
  var value = espacoId;
  console.log(urlRoutesEspacos + "/" + value);
  
  function mensagemdelete() {
      var name = confirm("Tem a certeza que pertende remover ?")
      if (name == true) {
          alert(value);
          fetch(urlRoutesEspacos + "/" + value, {
              method: 'DELETE',
              headers: { 'content-type': 'application/json' }
          })
...
function openSpaceDetailsModal(espacoId) {
  urltotal = urlRoutesEspacos + "/" + espacoId;
  async function fetchAsync() {
      const response = await fetch(urltotal);
      const espacos = await response.json();
      console.log(espacos);
      let id = document.getElementById("Idespaco");
      id.value = espacos.id_espaco;
      let nome = document.getElementById("Nome");
      nome.value = espacos.nome;
      let morada = document.getElementById("Morada");
      morada.value = espacos.morada;
      let localidade = document.getElementById("Localidade");
      localidade.value = espacos.localidade;
      let distrito = document.getElementById("Distrito");
      distrito.value = espacos.distrito;
      let cp = document.getElementById("CP");
      cp.value = espacos.codigoPostal;
      let lotacao = document.getElementById("Lotacao");
      lotacao.value = espacos.lotacao;
      let gestor = document.getElementById("Gestor");
      gestor.value = espacos.gestorEspacoId;

  }
  //chama a função fetchAsync()   
  fetchAsync().then(data => console.log("ok")).catch(reason => console.log(reason.message));
  showSpaceDetailsForm(espacoId);
  setTimeout(function () {
...
function editSpace() {
  var value = document.getElementById("Idespaco").value;
  function mensagemedita() {
      var name = confirm("Tem a certeza que pretende fazer as alterações ?")
      if (name == true) {
          ...
          fetch(urlRoutesEspacos + "/" + value, {
              ...
              body: JSON.stringify({
                  ...
                  id_espaco: document.getElementById("Idespaco").value,
                  ...
                  id_gestor_espaco: document.getElementById("Gestor").value
              })
...
async function preencherDadosUtilizador() {
  ...
  fetch('http://localhost:3000/api/v1/utilizadores/' + id_utilizador)
    .then(function(response) {
      //console.log(response.headers.get('Set-Cookie'));
      console.log(response);
      if (!response.ok) {
...
const refreshAtividades = async () => {
  ...
  fetch("http://localhost:3000/api/v1/atividades")
    .then(function(response) {
      //console.log(response.headers.get('Set-Cookie'));
      console.log(response);
      if (!response.ok) {
    ...
    .then(async function(result) {
      if (result) {
          for (const atividade of result) {
              ...
              const responseEspacos = await fetch(`http://localhost:3000/api/v1/espacos/` + id_espaco);
              const espaco = await responseEspacos.json();

              const responseReserva = await fetch(`http://localhost:3000/api/v1/reservas/espaco/` + id_espaco + '/atividade/' + id_atividade);
              const reserva = await responseReserva.json();

              const responseTipo = await fetch(`http://localhost:3000/api/v1/tipos_atividades/` + id_tipo_atividade);
...
async function popularListsCriarAtividadeModal() {
  ...
  const responseEspacos = await fetch(`http://localhost:3000/api/v1/espacos`);
  const espaco = await responseEspacos.json();
  ...
  const responseTiposAtividades = await fetch(`http://localhost:3000/api/v1/tipos_atividades`);
